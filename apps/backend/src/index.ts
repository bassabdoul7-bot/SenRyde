...
// --- CLEAN ROUTES (replaces broken tail) ---
...
app.patch("/trips/:id/status", async (req, res) => {
  try {
    ...
    const update: any = { status: statusRaw, updatedAt: new Date() };
    if (statusRaw === "completed") update.completedAt = new Date();

    const TripModel = (mongoose.models.Trip || Trip) as any;
    const trip = await TripModel.findByIdAndUpdate(id, update, { new: true }).lean();
...
// List users by role with simple search (?role=&q=)
app.get("/admin/users", async (req, res) => {
  try {
    ...
    const UserModel = (mongoose.models.User || User) as any;
    const users = await UserModel.find(filter).limit(100).lean();
    return res.json({ ok: true, users });
  } catch (e) {
    console.error("admin/users error", e);
    return res.status(500).json({ ok: false, error: "user_list_failed" });
  }
});
...
// List trips quickly
app.get("/debug/trips", async (_req, res) => {
  try {
    const TripModel = (mongoose.models.Trip || Trip) as any;
    const trips = await TripModel.find({}).limit(20).lean();
    res.json({ ok: true, count: trips.length, trips });
  } catch (e) {
    console.error("debug/trips", e);
    res.status(500).json({ ok: false, error: (e as Error).message });
  }
});

// Seed one driver + one pending trip
  ...
  try {
    ...
    if (!driver) {
      driver = await UserModel.create({
        name: "Ali Driver",
        email: `ali.driver.${Date.now()}@example.com`,
        role: "driver",
        password: await bcrypt.hash("Passw0rd!", 10),
      });
    }

    let rider = await UserModel.findOne({ role: "rider" });
    if (!rider) {
      rider = await UserModel.create({
        name: "Rita Rider",
        email: `rita.rider.${Date.now()}@example.com`,
        role: "rider",
        password: await bcrypt.hash("Passw0rd!", 10),
      });
    }
    ...
    res.json({
      ...
      note: "Use these IDs in PATCH calls"
    });
  } catch (e) {
    console.error("debug/seed", e);
    res.status(500).json({ ok: false, error: (e as Error).message });
  }
});
