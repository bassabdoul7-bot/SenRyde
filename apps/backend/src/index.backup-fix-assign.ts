...
app.patch("/trips/:id/assign", async (req, res) => {
  try {
    ...
    const TripModel: any = (mongoose.models.Trip || Trip);
    const update: any = {
      driver:   driverId,    // ObjectId-compatible
      driverId: driverId,    // string fallback
      status:   "accepted",
      updatedAt: new Date(),
    };
...
try {
  ...
  console.error("Trip schema augment failed:", e?.message || e);
}
